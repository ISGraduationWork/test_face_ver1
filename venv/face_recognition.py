...
# 顔のデータベーステーブルの作成(データベースがない場合生成)
c.execute('''CREATE TABLE IF NOT EXISTS faces
            (id INTEGER PRIMARY KEY, name TEXT, face_data BLOB, Registration_time TEXT)''')
...
def register_face():
    ...
    c.execute("INSERT INTO faces (name, face_data, Registration_time) VALUES (?, ?, ?)", (name, sqlite3.Binary(face_data), registered_at))
    conn.commit()
